...
function instant_domain_suggestion( $curl, $domain, $tld ) {
	...
	if ($curl->error) {
	    ...
	    $dn = (array) json_decode("[" . str_replace('"}' . "\n" . '{', '"},{', $curl->response ) . "]", true );
	    //print_r($dn);
	    if( count($dn) > 0 ) {
	    	foreach ($dn as $key => $value) {
	    		$dn_new[$value['tld']] = $value;
	    	}
	    	unset($dn);
	    	return $dn_new;
	    } else return false;
...
if ( $_SERVER["REQUEST_METHOD"] === "GET" && ( isset($_GET["domain"]) || isset($_GET['tld']) ) ) {
	...
	if( empty($domain_error) && empty($tld_error) ) {
		...
		if( strpos($tld, '.') == '0' ) {
			$inds_tld = ltrim($tld, '.') . ',' . implode(',', array_diff($tlds_without_dots, array(ltrim($tld, '.'))) );
		} else {
			$inds_tld = ltrim($tld, '.') . ',' . implode(',', array_diff($tlds_without_dots, array(ltrim($tld, '.'))) );
		}
		...
		if( ( $dn = instant_domain_suggestion( $curl, $domain, $inds_tld ) ) == false ) {
			...
			$dn['available'] = $dn[ltrim($tld, '.')]['isRegistered'] == 1 ? false : true;
		}
